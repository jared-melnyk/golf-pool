...
module GoldenValidationExporter
  ...
  def handicap_terms_section(format, round, fixture, scorecard)
    ...
    lines << "| **PH** | Playing Handicap | CH adjusted for **this game format** (allowance %). Used to allocate strokes hole by hole. |"
    lines << ""
    ...
    lines << "```"
    lines << ""
    lines << "**Playing Handicap** (#{FORMAT_LABELS.fetch(format)} uses **#{allowance}%** of CH)"
    lines << ""
    lines << "```"
    lines << "PH = round( CH × #{allowance}% )"
    lines << "```"
    lines << ""
    ...
    lines << "2. PH = round(#{example_ch} × #{allowance}%) = round(#{ph_step}) = **#{example_ph}**"
    lines << ""
    ...
    lines << "- If PH > 18: every hole gets **1 stroke**, plus an **extra stroke** on the (PH − 18) hardest holes."
    lines << ""
